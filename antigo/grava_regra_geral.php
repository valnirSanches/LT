<?php
for($result=1520;$result<=1540;$result++)
{
$a=1;
$b=1;
$c=1;
...
$aa=1;
	...
	for ($ii=1; $ii<26;$ii++)
	{
		$sql4  = "Select * FROM regras 
		 WHERE qt_repetiu = ".$qt[$ii][1]." and in_sair = '".$in_sair[$ii]."' and jogo1 = '".$valor[$ii][1]."' and jogo2 = '".$valor[$ii][2].
		...
		if(mysqli_affected_rows($mysqli) > 0){
		  $up = ("UPDATE regras SET qt_saiu=$dados4[qt_saiu]+1 WHERE nr_regra = $dados4[nr_regra]");
		  var_dump($up);
		  echo '<br>';
     	  mysqli_query($mysqli,$up);
		}else{
			
		  $sql3  = "Select max(nr_regra) as nr_regra FROM regras";
		 ...
		 $ins="INSERT INTO regras (qt_repetiu, nr_regra, in_sair, jogo1, jogo2, jogo3, jogo4, jogo5, jogo6, jogo7, jogo8, jogo9, jogo10, qt_saiu) VALUES (".$qt[$ii][1].",".(@$dados3[nr_regra]+1).",'".$in_sair[$ii]."','".$valor[$ii][1]."','".$valor[$ii][2]."','".$valor[$ii][3]."','".$valor[$ii][4]."','".$valor[$ii][5]."','".$valor[$ii][6]."','".$valor[$ii][7]."','".$valor[$ii][8]."','".$valor[$ii][9]."','".$valor[$ii][10]."', 1)";
			echo '<br>';
			print_r($ins);

<?php
for($result=1605;$result<=1710;$result++)
{
$a=1;
$b=1;
$c=1;
...
//	echo "<br>".$nr."-".$in_sair[$nr];
	...
	for ($ii=1; $ii<26;$ii++)
	{
		...
		if($in_sair[$ii]=='S'){
			$ins="INSERT INTO regras_individual (nr_regra, nr_concurso, nr_bola, qt_saiu) VALUES (".$dados4['nr_regra' ].",".$result.",".$ii.", 1)";
			echo '<br><br>';
			print_r($ins);
			mysqli_query($mysqli,$ins);
		}else{
			$ins="INSERT INTO regras_individual (nr_regra, nr_concurso, nr_bola, qt_nao_saiu) VALUES (".$dados4['nr_regra' ].",".$result.",".$ii.", 1)";
			echo '<br><br>';
			print_r($ins);
			mysqli_query($mysqli,$ins);
		}		
